...
func run():
    for id in ["witcheer"]:
        for facing in [-1.0, 1.0]:
            ...
            check(visual.animation_player.assigned_animation == "DefaultSwim" and visual.animation_player.is_playing(), "grounded swimming replaces resumed reaction")
            f.queue_free(); v.queue_free(); await process_frame
    var file = FileAccess.open("res://.verification/evidence/current_witcheer/coverage_runtime.json",FileAccess.WRITE)

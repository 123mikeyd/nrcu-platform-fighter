...
func run():
 for id in ["teknium","turbofit"]:
  ...
  for reason in ["reset","freeze","disable","shield","hit","landing","face","held","repeat","grab","stock"]:
   for k in [KEY_SPACE,KEY_S,KEY_D,KEY_A,KEY_F,KEY_E]:key(k,false)
   ...
   key(KEY_S,true);key(KEY_F,true);await frames(1);
 ...
 var file=FileAccess.open(OUT+"down_lifecycle.json",FileAccess.WRITE);file.store_string(JSON.stringify(rows,"  "));file.close()

...
const OUT = "res://.verification/core/sparring-lab/"
func run():
	var rows := []
	var policies := ["sparring_easy"]
	for kit in ["teknium","turbofit"]:
		for mode in ["grounded_jostle","legacy_solid"]:
			for policy in policies:
				...
				check(lab.set_p2_input_owner("sparring_easy"),"production Sparring route")
				...
				for i in 1200:
					...
					var frames: Dictionary = lab.sparring_inputs._frames
					var f = frames.get(2)
					if f == null: continue
				...
				check(openings>0,"actual close unlocked neutral opportunities "+kit+mode)

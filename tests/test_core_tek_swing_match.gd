...
func run():
	...
	var floor := StaticBody3D.new(); var collision := CollisionShape3D.new(); var box := BoxShape3D.new(); box.size = Vector3(30,1,10); collision.shape = box; floor.add_child(collision); floor.position.y = -.5; root.add_child(floor)
	var records := []
	for character in ["teknium"]:
		...
		if native:
			for c in [character,other]:
				var packed: PackedScene = load("res://assets/"+c+"/"+c+"_animations.glb")
				if c == "teknium": packed = preload("res://scripts/core/presentation/teknium_swing_source.gd").assembled_source(packed)
				var model = packed.instantiate(); world.add_child(model); views.append(model)
		...
		for ground in [true]:
			for vertical in [0]:
				for face in [-1,1]:
					var offset := Vector3(face*.95,0,0)
					if character == "turbofit" and not ground and vertical == 1: offset = Vector3(face*.4,-1.2,0)
					check(m.reset_with_collision_profiles({1:Vector3(0,.01,0),2:Vector3(0,.01,0)+offset},{1:profile(character),2:profile(other)}),"merged actual match")
					...
					for settle in 30: await step(m)
					check(a.runtime.grounded and b.runtime.grounded,"both actors actually supported by native floor")
	...
	if not failures: print("PASS: Tek swinging punch merged contact both facings (%d checks)" % checks)

...
func run():
 for player in [1,2]:
  for id in ["teknium","turbofit"]:
   ...
   for face in [1.0,-1.0]:
    for delay in [18,27,33]:
     for gap in [1.5,2.5,4.0,8.0]:
      for movement in ["stationary","toward","away"]:
       ...
       if movement!="stationary":
        var axis=face if movement=="away" else -face
 ...
 var file=FileAccess.open(OUT+"drop_default60_contacts.json",FileAccess.WRITE);file.store_string(JSON.stringify({"cases":rows,"checks":checks,"failures":failures},"  "));file.close()
 print("PASS DROP_DEFAULT60_COMPLETE cases=%d checks=%d failures=%d"%[rows.size(),checks,failures]);quit(1 if failures else 0)

...
func _ready():
 for i in 12:
  ...
  var mat=ShaderMaterial.new();mat.shader=preload("res://scripts/mephisto_leg_wisp.gdshader");mat.set_shader_parameter("seed",float(i)*3.71);mat.set_shader_parameter("opacity",.85);card.material_override=mat

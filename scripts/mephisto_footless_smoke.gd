...
func setup(v):
 ...
 for m in view.demon.find_children('*','MeshInstance3D',true,false):
  ...
  for s in source.get_surface_count():
   ...
   var mat=ShaderMaterial.new();mat.shader=preload('res://scripts/mephisto_leg_dissolve.gdshader')
   mat.set_shader_parameter('albedo_tex',old.albedo_texture);mat.set_shader_parameter('tint',old.albedo_color)
 ...
 for i in 28:
  ...
  var mat=ShaderMaterial.new();mat.shader=preload('res://scripts/mephisto_leg_wisp.gdshader');mat.set_shader_parameter('seed',float(i)*3.71);m.material_override=mat
...
func _process(delta):
 ...
 for i in cards.size():
  ...
  m.material_override.set_shader_parameter('opacity',sin(phase*PI)*lerpf(.72,.86,low))

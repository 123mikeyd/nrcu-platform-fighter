...
func _process(_delta: float):
    ...
    if is_instance_valid(scene) and scene.has_method("start_match"):
        active = (scene.setup == null or not scene.setup.visible) and not scene.match_over and is_instance_valid(scene.player_one) and scene.player_one.controls_enabled and scene.player_one.control_type == "human" and scene.player_one.player_index == 1

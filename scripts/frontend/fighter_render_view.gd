...
func _rebuild() -> void:
    ...
    for id in _ids:
        ...
        picture.texture = Portraits.portrait_texture(id)
...
func set_palette(index: int) -> void: _palette = index
...
func render_target() -> Texture2D:
    return Portraits.portrait_texture(_ids[0]) if not _ids.is_empty() else null

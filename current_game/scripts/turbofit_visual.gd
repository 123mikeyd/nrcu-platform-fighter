...
func begin_jump() -> void:
    # Called only after fighter accepts a jump; preserve the native Jump asset.
    if animation_player and animation_player.assigned_animation == "Jump":
...
func sync_pose(grounded: bool, motion: Vector3, interrupted: bool, shielding: bool, active_move: String, attack_direction: Vector3, facing: float, delta: float, attack_clip := "", attack_elapsed := 0.0, attack_duration := 1.0) -> void:
    ...
    if interrupted or shielding or not active_move.is_empty():
        ...
        if not grounded or motion.y > 0:
            ...
            if falling:
                ...
                falling = false
            # Passive landing cannot skate across the floor while movement is legal.
            # Stationary landing and all earlier attack-owned routes remain intact.
            if absf(motion.x) > 0.15:
                landing_remaining = 0.0
    ...
    if active_move == "SOUND WAVE":
        target_duration = 0.8 # provisional; shared Power Chord remains untouched

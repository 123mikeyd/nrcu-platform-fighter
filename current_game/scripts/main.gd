...
func start_match(slots: Array, teams: bool, bobo_encounter := false) -> bool:
    ...
    for i in range(slots.size()):
        ...
        if slot.kind == "empty":
            continue
        var fighter = _create_fighter(slot.character, bobo_encounter and i == 1, i)
...
func _story_playable_ids() -> Array[String]:
    ...
    ids.erase("ice_mage")
    ids.erase("bobo") # Incomplete player kit is freeplay-only; Story remains unchanged.
    return ids
...
func start_story() -> void:
    ...
    if story_state == "stage_complete":
        ...
        match_over = false
        bobo_health_bar.hide()
        story_title.text = "STORY 02 / ICE MAGE"
        story_detail.text = "Defeat the Normal Ice Mage. Watch for freezing Frost Bolts!\nBoth fighters start fresh with three stocks.\n\nA / D move · Space jump · F basic · G special\nAim with WASD · Esc back to setup"
        if get_node_or_null("/root/MobileTouch") and get_node("/root/MobileTouch").touch_mode:
            story_detail.text = "Defeat the Normal Ice Mage. Watch for freezing Frost Bolts!\nBoth fighters start fresh with three stocks.\n\nLeft pad: move / aim · Up: jump · Down: drop\nRight thumb: Attack / Special (hold to charge) · Setup: back"
        story_action.text = "START ENCOUNTER"
    ...
    if not selected is String or selected not in _story_playable_ids():
        ...
        for index in story_character.item_count:
            if story_character.get_item_metadata(index) == selected:
                ...
                break
    if story_encounter == 0: story_hero = selected
    slots[0].character = story_hero
    slots[0].kind = "human"
    slots[0].device = -1
    slots[1].character = "bobo" if story_encounter == 0 else "ice_mage"
    slots[1].kind = "bot"
    slots[1].difficulty = "normal"
    slots[2].kind = "empty"
    slots[3].kind = "empty"
    if start_match(slots, false, story_encounter == 0):
        story_state = "playing"
        hud_title.text = "STORY 01 — %s VS BOBO" % player_one.fighter_name
        hud_controls.text = "YOU / P1: WASD move & aim · Space jump · F basic · G special\nBobo: 400 HP · slow two-hit claws · punish his recovery! · Esc: pause"
        if story_encounter == 1:
            hud_title.text = "STORY 02 — %s VS ICE MAGE" % player_one.fighter_name
        ...
        player_two.reset_fighter(Vector3(0.6, 1.0, 0.0) if story_encounter == 0 else p2_spawn, true)
...
func _on_fighter_eliminated(_loser: CharacterBody3D) -> void:
    ...
    if story_state == "playing":
        ...
        story_state = ("stage_complete" if story_encounter == 0 else "complete") if won else "lost"
        story_title.text = "STAGE COMPLETE" if won else "TRY AGAIN"
        ...
        story_action.text = "NEXT ENCOUNTER" if won else "RETRY"
        if story_encounter == 1:
            story_detail.text = "TWO-ENCOUNTER SLICE COMPLETE\nBobo and Ice Mage defeated.\nRestart the run or return to match setup." if won else "Out of stocks! Ice Mage wins this encounter.\nRetry with three fresh stocks for both fighters."
            story_action.text = "RESTART RUN" if won else "RETRY"
...
func _create_fighter(id: String, boss: bool, _slot: int):
    return load("res://scripts/bobo_fighter.gd").new() if boss else (load("res://scripts/bobo_player.gd").new() if id == "bobo" else FighterScript.new())

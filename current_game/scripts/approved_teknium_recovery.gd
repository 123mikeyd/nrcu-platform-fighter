...
func before_tick(delta):
 ...
 if lab_move=="side_basic":
  var raw=actor._read_raw_controls(0)
  var tapped: bool = actor._sample_short.has("attack")
  var aim: Vector2 = actor._sample_short.get("attack", Vector2(int(raw.right)-int(raw.left),int(raw.down)-int(raw.up)))
  var direction_ok: bool = (absf(aim.y)<=.1 and aim.x*lab_facing>=-.1) if tapped else (not raw.up and not raw.down and not (raw.left and lab_facing>0) and not (raw.right and lab_facing<0))
  if ((raw.attack and not side_attack_held) or tapped) and side_step<2 and lab_time>=SIDE_BUFFER_OPEN and direction_ok and not raw.shield and not raw.special and not raw.jump:side_buffered=true

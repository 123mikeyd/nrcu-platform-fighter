...
func _ready() -> void:
    Input.joy_connection_changed.connect(_short_device_changed)
    ...
    if character_id == "doge_man":
        ...
        doge_cape_attack = preload("res://scripts/doge_cape_attack.gd").new()
        doge_cape_attack.name = "DogeCapeAttack"
        add_child(doge_cape_attack)
    ...
    if character_id == "doge_man":
        var doge_hurtboxes = preload("res://scripts/doge_body_hurtboxes.gd").new()
        doge_hurtboxes.name = "BodyHurtboxes"
        add_child(doge_hurtboxes)
...
func coordinate_state():
    state_coordinator.reconcile(self)
    return state_coordinator

# One-sample event supplement; never a recovery queue. Holds remain snapshots.
var _pending_short: Dictionary = {}
var _sample_short: Dictionary = {}
var _short_down: Dictionary = {}
var _short_suppressed: Dictionary = {}

func _short_action(event: InputEvent) -> String:
    if control_type != "human": return ""
    if input_device >= 0:
        if event is InputEventJoypadButton and event.device == input_device:
            return {JOY_BUTTON_X: "attack", JOY_BUTTON_B: "special", JOY_BUTTON_A: "jump", JOY_BUTTON_DPAD_UP: "up", JOY_BUTTON_DPAD_LEFT: "left", JOY_BUTTON_DPAD_RIGHT: "right", JOY_BUTTON_DPAD_DOWN: "down"}.get(event.button_index, "")
        return ""
    if player_index not in [1, 2] or not event is InputEventKey: return ""
    var bindings := {KEY_F: "attack", KEY_G: "special", KEY_SPACE: "jump", KEY_W: "up", KEY_A: "left", KEY_D: "right", KEY_S: "down"} if player_index == 1 else {KEY_K: "attack", KEY_L: "special", KEY_ENTER: "jump", KEY_UP: "up", KEY_LEFT: "left", KEY_RIGHT: "right", KEY_DOWN: "down"}
    return bindings.get(event.keycode, "")

func _input(event: InputEvent) -> void:
    var action := _short_action(event)
    if action.is_empty(): return
    if not event.pressed:
        if action == "special" and charging and _short_down.has(action) and not _short_suppressed.has(action) and _short_eligible(action):
            _pending_short["special_release"] = true
        _short_down.erase(action)
        _short_suppressed.erase(action)
    elif not event.is_echo() and not _short_down.has(action):
        # GUI-consumed downs cannot become new combat actions via polling.
        _short_suppressed[action] = true
...
func _short_eligible(action: String) -> bool:
    ...
    if action == "special" and character_id == "ggb" and drop_committed: return true
    if landing_lag > 0 or _torpedo_committed(): return false
    if action == "special" and not doge_attack_clip.is_empty(): return true
    # Jump/store direction permission is not generic attack recovery permission.
...
func _unhandled_input(event: InputEvent) -> void:
    var action := _short_action(event)
    if action.is_empty() or event.is_echo() or not event.pressed: return
    if _short_down.has(action): return
    _short_down[action] = true
    # Revival's existing hold-driven departure is not an ordinary combat sample.
    # Do not capture a tap there, but do not steal its held jump/attack either.
    if revival and revival.phase != "idle" and controls_enabled:
        _short_suppressed.erase(action)
        return
    if not _short_eligible(action): return
    _short_suppressed.erase(action)
    if action in ["left", "right", "down"] and not charging: return
    if not _pending_short.has(action):
        var held := _read_raw_controls(0)
        _pending_short[action] = Vector2(int(held.right)-int(held.left), int(held.down)-int(held.up))

func _clear_short_input() -> void:
    _pending_short.clear()
    _sample_short.clear()
    _short_down.clear()
    _short_suppressed.clear()
    if not is_inside_tree() or control_type != "human": return
    var held := _read_raw_controls(0)
    for action in ["attack", "special", "jump", "up"]:
        if held[action]:
            _short_suppressed[action] = true
            _short_down[action] = true

func _short_device_changed(device: int, _connected: bool) -> void:
    if device == input_device: _clear_short_input()

func _notification(what: int) -> void:
    if what in [NOTIFICATION_PAUSED, NOTIFICATION_UNPAUSED, MainLoop.NOTIFICATION_APPLICATION_FOCUS_OUT, MainLoop.NOTIFICATION_APPLICATION_FOCUS_IN, MainLoop.NOTIFICATION_APPLICATION_PAUSED, MainLoop.NOTIFICATION_APPLICATION_RESUMED]:
        _clear_short_input()
...
func _physics_process(delta: float) -> void:
    ...
    if coordinate_state().control_lane() == "disabled":
        if ready_settling:
            velocity.x = 0
    ...
    var jump_down: bool = input.jump or (up_down and not _short_suppressed.has("up"))
    ...
    # One decision owns this dispatch; existing handlers retain their clocks,
    # input order, collision, animation and numeric movement policy.
    var decision = coordinate_state()
    ...
    if control_lane == "frozen":
        ...
        elif not is_instance_valid(caught_by.actor) or not caught_by.actor.controls_enabled or caught_by.actor.freeze_remaining > 0:
            cancel_magic()
        else:
    ...
    elif control_lane == "casting":
        velocity.x = move_toward(velocity.x, 0, GROUND_ACCELERATION * delta)
    elif control_lane == "hitstun":
        hitstun -= delta
    elif control_lane == "counter":
        velocity.x = 0
        ...
        var move_axis: float = decision.intent_axis(float(int(right_down) - int(left_down)))
        ...
        var attack_aim: Vector2 = _sample_short.get("attack", aim)
        var witcheer_turn_edge := character_id == "witcheer" and (_sample_short.has("attack") or (attack_down and not _attack_was_down)) and attack_aim.x * facing < -0.1 and absf(attack_aim.y) <= 0.1
        var acceleration := GROUND_ACCELERATION if is_grounded() else AIR_ACCELERATION
        velocity.x = move_toward(velocity.x, (move_axis * MOVE_SPEED * ground_speed_multiplier() if decision.movement_allowed() else 0.0), acceleration * delta)
        ...
        if charging:
            ...
            for direction in ["left", "right", "up", "down", "jump"]:
                if (input[direction] and not _charge_store_held.get(direction, false)) or _sample_short.has(direction):
                    store_edge = true
            ...
            if teknium_specials and store_edge:
                ...
                if not special_down or _sample_short.has("special_release"):
                    release_special()
        elif not shielding and not (doge_ground_rush and doge_ground_rush.phase != "idle"):
            # Down owns pass-through support before the same attack edge routes.
            if down_down and not _down_was_down:
                try_drop_through()
            # Attack chords still take priority over tap-up jumping.
            if _sample_short.has("attack") or (attack_down and not _attack_was_down):
                basic_attack(_sample_short.get("attack", aim), not is_grounded())
            elif _sample_short.has("special") or (special_down and not _special_was_down):
                start_special(_sample_short.get("special", aim))
            elif (_sample_short.has("jump") or _sample_short.has("up") or (jump_down and not _jump_was_down)) and not attack_down and not special_down:
                ...
                if input.jump or _sample_short.has("jump") or doge_attack_clip != "TysonTwoPiece":
                    try_jump()
    ...
    if doge_cape_attack: doge_cape_attack.after_tick(delta)
...
func _read_raw_controls(delta: float) -> Dictionary:
    ...
    for action in _short_suppressed:
        if action in ["attack", "special", "jump"]: result[action] = false
...
func try_jump() -> bool:
    ...
    if character_id == "turbofit" and _visual_root:
        _visual_root.get_node("TurboFitVisual").begin_jump()
...
func start_special(aim: Vector2) -> void:
    ...
    if character_id == "ggb" and absf(aim.y) <= 0.1:
        ...
        goo.source = self
        goo.direction = facing
        goo.color = body_color
        goo.sound_wave = false
        get_parent().add_child(goo)
        goo.global_position = global_position + Vector3(facing * 0.8, 1, 0)
        if _visual_root:
            goo.global_position.y = _visual_root.get_node("GGBVisual").body_center_world().y
        attack_cooldown = 0.55
        last_move = "STICKY GOO"
        return
...
func _update_move_visuals(delta := 0.0, interrupted := false) -> void:
    ...
    if _visual_root:
        ...
        if doge_visual:
            ...
            # Release their skeletal override when airborne control returns;
            # retain the full approved reaction on the ground.
            if fitted_reaction and fitted_reaction.active and hitstun <= 0 and not is_grounded():
                fitted_reaction.clear()
            _visual_root.scale = Vector3.ONE
            _visual_root.rotation = Vector3.ZERO
            var attack_duration := float(doge_attack_timings[doge_attack_clip].duration) if not doge_attack_clip.is_empty() else 1.0
            if air_doge and air_doge.active():
                air_doge.present()
            ...
            elif doge_cape_attack and doge_cape_attack.active:
                doge_cape_attack.present(doge_cape_attack.elapsed)
...
func basic_attack(aim: Vector2, airborne: bool) -> void:
    ...
    if character_id == "doge_man" and not airborne:
        if absf(aim.y) <= 0.1:
            ...
            return
        if aim.y > 0.1:
            doge_cape_attack.start()
            return
        _start_doge_attack("Uppercut", aim)
        return
...
func _guard_held_sound_orb() -> void:
    if not _sound_orb_held: return
    if hitstun > 0 or freeze_remaining > 0 or magic_locked() or not controls_enabled or stocks <= 0 or is_instance_valid(caught_by) or (tumble and tumble.active) or not is_grounded():
        _clear_sound_orb()
        return
    # Read held human controls before hitstop/tumble/disabled early returns.
    # This never starts an activation or clears the per-activation ledgers.
    if control_type == "human":
        var held := _read_raw_controls(0.0)
        if not held.down or not held.special or held.left or held.right or held.up or held.jump or held.attack:
            _clear_sound_orb()
...
func _cancel_doge_attack(preserve_up_pose := false) -> void:
    if air_doge: air_doge.cancel()
    if doge_cape_attack: doge_cape_attack.cancel()
    if doge_air_drop: doge_air_drop.cancel()
...
func _directional_hit(hit_damage: float, base_knockback: float, attack_range: float, direction: Vector3, cooldown: float) -> void:
    # Turbo's up-basic launches vertically, but strikes forward and up.
    # Feet-to-feet offsets on level ground have zero dot with Vector3.UP;
    # using the launch vector as the contact cone made grounded foes unhittable.
    var contact_direction := direction
    if character_id == "turbofit" and last_move == "GUITAR SWING" and direction == Vector3.UP:
        contact_direction = Vector3(facing, 1.0, 0.0).normalized()
    ...
    for target in get_tree().get_nodes_in_group("fighters"):
        if not can_hit(target):
            continue
        var offset: Vector3 = target.global_position - global_position
        if absf(offset.z) < 1.5 and offset.length() <= attack_range and offset.normalized().dot(contact_direction) > 0.4:

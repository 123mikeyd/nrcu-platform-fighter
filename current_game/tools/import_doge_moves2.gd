...
func _post_import(scene: Node) -> Object:
    ...
    if document.append_from_file(get_source_file(), kick_state) == OK:
        ...
        for clip in ["GoalkeeperKick", "AirDoge_GROUND_SUCCESS", "AirDoge_GROUND_MISS", "AirDoge_AIR_SUCCESS", "AirDoge_AIR_MISS"]:
            if kick_player.has_animation(clip):
                if library.has_animation(clip): library.remove_animation(clip)
                library.add_animation(clip, kick_player.get_animation(clip).duplicate(true))
        kick_scene.free()
    return scene

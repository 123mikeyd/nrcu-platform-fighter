...
const ArmTiming=preload("res://story_boss/scripts/mephisto_arm_timing.gd")
var actor
...
func chain_animation():
 if not chain_pose:
  chain_pose=preload("res://story_boss/scripts/mephisto_chain_pose.gd").new();chain_pose.moves=self
 return chain_pose
func kick():
 if not girl_kick:
  girl_kick=preload("res://story_boss/scripts/mephisto_girl_kick.gd").new();girl_kick.moves=self
 return girl_kick
func magic():
 if not girl_magic:
  girl_magic=preload("res://story_boss/scripts/mephisto_girl_magic.gd").new();girl_magic.moves=self;add_child(girl_magic)
 return girl_magic
const BASIC={
...
func start_kit(id:String,direction:float):
 ...
 if id=="ShadowChain":
  shadow_chain=preload("res://story_boss/scripts/mephisto_shadow_chain.gd").new();shadow_chain.moves=self;actor.get_parent().add_child(shadow_chain)
func duration()->float:
 if move in ["ForwardBackhand","GroundPalmSlam"]:return ArmTiming.duration(move)
 if move=="ShadowChain":return 100000.0
 if move=="ShadowRecover":return preload("res://story_boss/scripts/mephisto_chain_pose.gd").RETRACT
 if move=="AnkleRake":return preload("res://story_boss/scripts/mephisto_girl_kick.gd").DURATION
...
func tick(delta:float):
 ...
 if move in ["ForwardBackhand","GroundPalmSlam"]:
  ...
  if elapsed>=.12 and not impulse_done:
   ...
   effect=preload("res://story_boss/scripts/mephisto_orange_smoke.gd").new()
 ...
 if move=="GroundPalmSlam" and time>=ArmTiming.time_at(move,d.hit) and actor.is_grounded():
  ...
  if not floor_hit.is_empty() and floor_hit.normal.y>.7 and absf(floor_hit.position.y-actor.global_position.y)<.14:
   if not impulse_done:
    ...
    effect=preload("res://story_boss/scripts/mephisto_palm_dust.gd").new();actor.get_parent().add_child(effect);effect.global_position=smoke_origin
